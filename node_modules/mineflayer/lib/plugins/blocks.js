...
function floor(vectofloor) {
  return new Vec3(Math.floor(vectofloor.x), Math.floor(vectofloor.y), Math.floor(vectofloor.z));
}
...
function inject (bot, { version }) {
  ...
  function posInChunk (pos) {
    return floor(pos).modulus(new Vec3(16, 256, 16))
  }
  ...
  bot._client.on('multi_block_change', (packet) => {
    ...
    for (let i = 0; i < packet.records.length; ++i) {
      ...
      const pt = new Vec3(packet.chunkX * 16 + blockX, record.y, packet.chunkZ * 16 + blockZ)
